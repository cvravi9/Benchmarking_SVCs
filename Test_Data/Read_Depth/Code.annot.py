...
dff = pd.read_csv("Updated_Test.annot.vcf", sep = '\t', index_col= False)
...
dff[['DP', 'AF', 'DP4', 'HRUN', 'SB', 'INDEL', 'EFF', 'LOF', 'T']] = dff['INFO'].str.split(';',expand=True)
...
dff.to_csv('Test.annot_DP_Values.csv', sep=',', index = None)
...
dk = dff['Read_Depth']
dkk = dk.hist()
dkk.figure.savefig('Test.annot_Histogram.png', dpi = 300)
