...
dff = pd.read_csv("Miracum_AF_0.7.frq", sep = '\t', index_col= False)
...
dff1 = pd.read_csv("Somatic_AF_0.4.frq", sep = '\t', index_col= False)
...
result.columns = ['CHROM-POS', 'Miracum_AF_0.7_REF', 'Somatic_AF_0.4_REF']
print(result)

result["REF_Comparision"] = result["Miracum_AF_0.7_REF"].equals(result["Somatic_AF_0.4_REF"])
print(result)

result.to_csv('0.7_vs_0.4_AF_Comparision.csv', sep='\t', index = None)
