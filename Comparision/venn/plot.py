...
#
# Title abbreviations:
# 	 0 .. first .. first.vcf.gz
# 	 1 .. secon .. second.vcf.gz
# 	 2 .. first + secon .. first.vcf.gz
#
...
if plot_venn_snps:
	...
	ax1.bar([1,2,3],[0,16815,191],align='center',color='orange',width=0.3)
	ax1.ticklabel_format(style='sci', scilimits=(0,0), axis='y')
	ax1.set_xlim(0.5,3.5)
	plt.xticks([1,2,3],('first\nts/tv 0.00\n0','first + secon\nts/tv 2.37\n16,815','secon\nts/tv 1.08\n191'))
...
if plot_venn_indels:
	fig = plt.figure(figsize=(4.33070866141732,3.93700787401575))
	ax1 = fig.add_subplot(111)
	ax1.bar([1,2,3],[0,2307,77],align='center',color='red',width=0.3)
	ax1.ticklabel_format(style='sci', scilimits=(0,0), axis='y')
	ax1.set_xlim(0.5,3.5)
	plt.xticks([1,2,3],('first\n0','first + secon\n2,307','secon\n77'))
...
if plot_SNP_count_by_af:
	...
	if 0 in dat and len(dat[0])>2:
		ax1.plot([row[0] for row in dat[0]], [row[1] for row in dat[0]], '-o',markersize=3, color='orange',mec='orange',label='first')
		has_data = 1

	if 1 in dat and len(dat[1])>2:
		ax1.plot([row[0] for row in dat[1]], [row[1] for row in dat[1]], '-o',markersize=3, color='red',mec='red',label='secon')
		has_data = 1

	if 2 in dat and len(dat[2])>2:
		ax1.plot([row[0] for row in dat[2]], [row[1] for row in dat[2]], '-o',markersize=3, color='darkgreen',mec='darkgreen',label='first + secon')
		has_data = 1
...
if plot_SNP_overlap_by_af and len(dat)>1:
	...
	ax1.set_ylabel('Fraction found in secon [%]')
	ax1.set_xscale('log')
	ax1.set_xlabel('Non-reference allele frequency in first')
	ax1.set_xlim(0,1.01)
...
if plot_dp_dist:
	...
	if pdf_plots: plt.savefig('depth.2.pdf')
	plt.close()
