...
df = pd.read_csv("Strelka_Somatic_0.4.csv", sep = '\t', index_col= False)
df1 = pd.read_csv("VarScan_Somatic_0.4.csv", sep = '\t', index_col= False)

# Merging columns based on "CHROM-POS"
First = pd.merge(df, df1, on=['ALT', 'REF'])

# Mentioning the column names and inputing the csv file.
First.columns = ['REF', 'ALT', 'Strelka_Somatic_0.4', 'VarScan_Somatic_0.4']
print(First)
